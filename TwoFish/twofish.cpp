...
void twofish::initialize(std::string const &key) {
...
twofish::twofish(std::string const &key) { initialize(key); }

uint32_t twofish::F(uint32_t x) {
  uint16_t a, b, c, d;
  uint32_t y;
...
void twofish::encrypt(uint32_t &xl, uint32_t &xr) {
  uint32_t Xl = xl;
  uint32_t Xr = xr;
...
void twofish::decrypt(uint32_t &xl, uint32_t &xr) {
  uint32_t Xl = xl;
  uint32_t Xr = xr;
...
int main(int argc, char const *argv[]) {
  std::string key("150977a3beb13d810c16be71707a3b4d");
  std::string message("My name is Vikhyat and i am also a student of CSE-AI");
  std::string cipher("");
  ...
  len = message.length();
  twofish twofish(key);
  std::cout << "Plaintext is: " << message << std::endl;
  uint32_t lm, rm;
  for (size_t i = 0; i < len; i += 8) {
    ...
    twofish.encrypt(lm, rm);
    cipher += from_uint(lm) + from_uint(rm);
  }
  // Convert the cipher value encoding to base64
  std::string decipher("");
  std::cout << "Ciphertext is: " << cipher << std::endl;
  ...
  for (size_t i = 0; i < len; i += 8) {
    ...
    twofish.decrypt(lm, rm);
    decipher += from_uint(lm) + from_uint(rm);
  }

  std::cout << "Decrypted text is: " << decipher << std::endl;
  if (message == decipher) {
    return 0;
  } else {
    return 1;
