...
def driver():
    ...
    encrypt_text = input("Enter data to encrypt: ")
    encrypt_bytes = encrypt_text.encode('utf-8')  # Convert string to bytes
...
def encrypt(input_text):
    ...
    input_hex = binascii.hexlify(input_text.encode())  

    # Split hex into two 32-bit halves    
    left = int(input_hex[0:8], 16)
    right = int(input_hex[8:16], 16)  
    data = (left << 32) | right

    # Encryption 
    encrypted_data = encryption(data)
    
    # Convert encrypted data back to hex
    encrypted_hex = hex(encrypted_data)[2:].rstrip("L").zfill(16)
    return encrypted_hex

def decrypt(input_hex):  
    # Split encrypted hex into two 32-bit halves
    left = int(input_hex[0:8], 16) 
    right = int(input_hex[8:16], 16)
    data = (left << 32) | right
    ...
    # Combine decrypted halves back to hex
    decrypted_hex = hex(decrypted_data)[2:].rstrip("L").zfill(16)
