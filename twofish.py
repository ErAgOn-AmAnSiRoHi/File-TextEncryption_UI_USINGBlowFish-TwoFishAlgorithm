...
# Ask user if they want to read from file or input from console

# method = input("Enter the method (File/Console) : ")
...
#     # If length of key is more than 32 bytes, then invalid key

#     if(len(key)>32):
#         print("Invalid key")
#         exit()
        
#     # If length of key is less than 32 bytes, then pad the key with null bytes

#     else:
#         key = key.zfill(32)

#     if typ.lower() == "encrypt":
#         # Take plaintext as input from user and convert it to hex

#         plaintext = input("Enter the plaintext : ")
#         plaintext = plaintext.encode('utf-8').hex()

#         # If length of plaintext is more than 32 bytes, then firstly split the plaintext into 32 bytes blocks and then encrypt each block and store each ciphertext in a list
...
#         # If length of plaintext is less than 32 bytes, then pad the plaintext with null bytes and encrypt it
...
#         # Now for decryption, if length of ciphertext is more than 32 bytes, then firstly split the ciphertext into 32 bytes blocks and then decrypt each block and store each decrypted text in a list
...
#             decrypted = decrypted.lstrip('\x00')
#             print("The Decoded plaintext is : ")
#             print(decrypted)   
# else:
...
#     # if typ.lower() == "encrypt":
#         # Read the plaintext from the file and convert it to hex
...
#                     block = block.zfill(32)
#                 ciphertext_blocks.append(encrypt(block,key))
#             # Print the ciptertext to the file
#             open('ciphertext.txt','w').write(''.join(ciphertext_blocks))
...
#             # Remove the padding from each decrypted block
#             decrypted_blocks = [block.lstrip('\x00') for block in decrypted_blocks]
#             # Print the decrypted text in a new file named decrypted.txt
#             open('decryptedtext.txt','w').write(''.join(decrypted_blocks))
...
#             # remove the padding from the decrypted text
#             decrypted = decrypted.lstrip('\x00')
#             # Print the decrypted text to the file
#             open('decryptedtext.txt','w').write(decrypted)